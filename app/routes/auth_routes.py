...
from app.auth import login_required, role_required, developer_required
...
def seed_default_users():
    """
    初始化默认用户账号（启动时自动调用）。
    如果数据库中没有任何用户，创建四个默认账号（每类角色一个）。
    """
    if User.query.count() == 0:
        users_data = [
            ('admin', '超级管理员', 'super_admin', 'admin123'),
            ('developer', '装备开发人员', 'developer', '123456'),
            ('process', '工艺人员', 'process', '123456'),
            ('operator', '操作人员', 'operator', '123456'),
        ]
...
def create_user():
    ...
    请求体:
        ...
        password: 密码（必填）
        role: 角色（operator/process/developer/super_admin，默认 operator）
    """
    data = request.get_json() or {}
    username = data.get('username', '').strip()
    ...
    if role not in ROLE_HIERARCHY:
        return jsonify({'code': 1, 'message': '无效的角色类型'}), 400
